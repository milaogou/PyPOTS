"""
The implementation of HELIX_NoFusion for the partially-observed time-series imputation task.
"""
...
from .core import _HELIX_NoFusion
from .data import DatasetForHELIX_NoFusion
...
class HELIX_NoFusion(BaseNNImputer):
    """The PyTorch implementation of the HELIX_NoFusion model (Ablation Study).
    ...
    def _print_model_configuration(self):
        if self.verbose:
            logger.info("=" * 60)
            logger.info(f"HELIX_NoFusion Model Configuration (Ablation):")
            logger.info("=" * 60)
    ...
    def fit(
        ...
        train_dataset = DatasetForHELIX_NoFusion(
        ...
        if val_set is not None:
            ...
            val_dataset = DatasetForHELIX_NoFusion(
