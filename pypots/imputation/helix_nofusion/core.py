"""
The core wrapper assembles the submodules of HELIX_NoFusion imputation model.
...
class BackboneHELIX_NoFusion(nn.Module):
    """
    HELIX backbone WITHOUT multi-level fusion (ABLATION).
    Uses only the last layer's output.
    """
    ...
    def forward(self, X, missing_mask):
        """
        ABLATION: No global fusion - only use last layer output
        """
        # Embedding
        ...
        # ABLATION: Do NOT store intermediate outputs for global fusion
        ...
        for layer_encoders in self.encoders:
            ...
            # Intra-layer fusion (keep this for fair comparison)
            x = torch.stack([time_encoded, feat_encoded, time_feat, feat_time], dim=0).mean(dim=0)
        
        # ABLATION: No global fusion - directly use the last layer output
...
class _HELIX_NoFusion(ModelCore):
    """Core model wrapper for HELIX_NoFusion."""
    ...
    def __init__(
        ...
        self.backbone = BackboneHELIX_NoFusion(
