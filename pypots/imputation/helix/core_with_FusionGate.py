...
class BackboneHELIX(nn.Module):
    ...
    def __init__(self, n_features, pe_dim, feature_embed_dim, d_model, n_heads, n_layers, dropout):
        ...
        self.fusion_gate = FusionGate(d_model, n_inputs=4 * n_layers + 1)
    ...
    def forward(self, X, missing_mask):
        ...
        fused = self.fusion_gate(all_outputs)
...
class FusionGate(nn.Module):
    """Gating mechanism to combine multiple inputs."""
    ...
    def __init__(
        ...
        self.gate = nn.Linear(d_single_input * n_inputs, n_inputs)
        self.softmax = nn.Softmax(dim=-1)

    def forward(self, inputs):
        concatenated = torch.cat(inputs, dim=-1)
        gate_weights = self.softmax(self.gate(concatenated))
        output = sum(w.unsqueeze(-1) * inp for w, inp in zip(gate_weights.unbind(-1), inputs))
        return output
