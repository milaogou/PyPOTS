"""
The core wrapper assembles the submodules of HELIX_NoFeatureEmbed imputation model.
...
class TimeSeriesEmbedding2D(nn.Module):
    """Embedding layer WITHOUT feature identity embedding (ABLATION)."""
    
    def __init__(self, n_features, pe_dim=16, feature_embed_dim=1):
        super().__init__()
        self.n_features = n_features
        self.pe_dim = pe_dim
        # ABLATION: feature_embed_dim kept for compatibility but not used
        
        # Rotary positional encoding for temporal dimension
        self.temporal_pe = RotaryPositionalEncoding(d_model=pe_dim)
        
        # ABLATION: No learnable feature identity embedding
    ...
    def forward(self, X, missing_mask):
        ...
        embedded : tensor, shape [B, T, F, embed_dim]
            ABLATION: embed_dim = 1 + pe_dim + 1 (removed feature_embed_dim)
        """
        B, T, F = X.shape
        device = X.device
        ...
        # ABLATION: Feature identity embedding removed
        
        # Missing mask [B, T, F, 1]
        mask_feature = missing_mask.unsqueeze(-1)
        
        # Concatenate: data_val + temporal_encoding + mask
        embedded = torch.cat([
            data_val,           # [B, T, F, 1]
...
class BackboneHELIX_NoFeatureEmbed(nn.Module):
    """
    HELIX backbone WITHOUT feature identity embedding (ABLATION).
    """
    
    def __init__(self, n_features, pe_dim, feature_embed_dim, d_model, n_heads, n_layers, dropout):
        super().__init__()
        
        # ABLATION: embed_dim = pe_dim + 2 (no feature_embed_dim)
        embed_dim = pe_dim + 2
    ...
    def forward(self, X, missing_mask):
        """Forward pass"""
        # Embedding (without feature identity)
...
class _HELIX_NoFeatureEmbed(ModelCore):
    """Core model wrapper for HELIX_NoFeatureEmbed."""
    ...
    def __init__(
        ...
        self.backbone = BackboneHELIX_NoFeatureEmbed(
            n_features=n_features,
            pe_dim=pe_dim,
            feature_embed_dim=feature_embed_dim,  # Kept for compatibility
            d_model=d_model,
            n_heads=n_heads,
            n_layers=n_layers,
            dropout=dropout
