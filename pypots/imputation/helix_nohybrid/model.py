"""
The implementation of HELIX_NoHybrid for the partially-observed time-series imputation task.
"""
...
from .core import _HELIX_NoHybrid
from .data import DatasetForHELIX_NoHybrid
...
class HELIX_NoHybrid(BaseNNImputer):
    """The PyTorch implementation of the HELIX_NoHybrid model (Ablation Study).
    ...
    def _print_model_configuration(self):
        if self.verbose:
            logger.info("=" * 60)
            logger.info(f"HELIX_NoHybrid Model Configuration (Ablation):")
            logger.info("=" * 60)
    ...
    def fit(
        ...
        train_dataset = DatasetForHELIX_NoHybrid(
        ...
        if val_set is not None:
            ...
            val_dataset = DatasetForHELIX_NoHybrid(
